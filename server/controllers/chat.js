...
const addMembers = TryCatch(async (req, res, next) => {
  ...
  if (String(chat.creator) !== String(req.user))
...
const leaveGroup = TryCatch(async (req, res, next) => {
  ...
  emitEvent(req, ALERT, chat.members, `User ${user.naem} has left the group `);
...
const sendAttachments = TryCatch(async (req, res, next) => {
  ...
  // Accessing the uploaded files from the request
...
const deleteChat = TryCatch(async (req, res, next) => {
  ...
  if (String(chat.groupChat && chat.creator) !== String(req.user))
    return next(
